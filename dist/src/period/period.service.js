...
let PeriodService = class PeriodService {
    ...
    async comparePeriods(periodId1, periodId2) {
        const getPeriodData = async (periodId) => {
            const period = await this.prisma.periodHeader.findUnique({
                where: { id: periodId },
                include: { journals: { include: { lines: true } } },
            });
            if (!period)
                throw new common_1.NotFoundException("الفترة غير موجودة");
            let netProfit = 0;
            if (period.isClosed) {
                const profitAccruals = await this.prisma.partnerPeriodProfit.findMany({
                    where: { periodId },
                });
                netProfit = profitAccruals.reduce((sum, p) => sum + Number(p.totalProfit || 0), 0);
                ...
                const companyProfit = companyShareLines.reduce((sum, line) => sum + Number(line.credit || 0), 0);
                ...
                netProfit = accruals.reduce((sum, a) => sum +
                    Number(a.partnerFinal || 0) +
                    Number(a.companyCut || 0), 0);
            }
