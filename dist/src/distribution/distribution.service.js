...
let DistributionService = class DistributionService {
    ...
    async postClosing(periodId, userId, savingAmountInput) {
        ...
        if (savingAmountInput && savingAmountInput > 0) {
            const totalPartnersProfit = accruals.reduce((sum, a) => sum + Number(a.totalProfit), 0);
            if (totalPartnersProfit <= 0)
                throw new common_1.BadRequestException('إجمالي أرباح الشركاء غير صالح');
